...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  if(cell.accessoryType==UITableViewCellAccessoryNone){
    ...
    cell.accessoryType = SIGEV_NONE;
    [_selectedIndices removeObject:indexPath];
  }
  
  NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:nil ascending:YES];
  NSArray *sortedArray = [_selectedIndices sortedArrayUsingDescriptors:@[sortDescriptor]];
  
  _selectedIndices = [[NSMutableArray alloc] initWithArray:sortedArray];
...
-(void)pressDoneButton
{
  ...
  for(NSIndexPath *array in _selectedIndices){
    [_selectedAssets addObject:_assets[array.row]];
  }
